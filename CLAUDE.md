...
| `internal/ssm` | SSM Parameter Store client (Get/Put/List/History/Describe) + `FakeClient`. Not a `storage.Client`. |
...
**SSM Parameter Store** (`cmd/ladle/ssm.go`, `runSSM`): early-branch dispatcher for `ssm://`, mirroring the S3 flows (edit / pipe out / pipe in / list / versions / meta) against `internal/ssm.Client`. Differences from S3: no bucket (the URI's `Key` is the full parameter name, always leading-slash normalized); no TUI browser (directory URIs — and names that are really namespace prefixes — print a listing to stdout, sorted, one canonical `ssm://` URI per line); `--versions` prints tab-separated history to stdout. `internal/ssm.Client.Describe` reads metadata via `GetParameterHistory` (ARN-scopable IAM + strongly consistent), NOT `DescribeParameters` (account-wide + eventually consistent). Because SSM has no metadata-only write API, `--meta` re-`Put`s the current value alongside the new attributes; `Put` only sends `KeyId` for SecureString. **SecureString safety (`--reveal`):** value-exposing operations (edit, pipe-out, update diff) refuse for SecureString unless `--reveal` is passed; `--yes` writes skip the diff and don't need `--reveal`, but the no-op check still runs for non-secure values. Pipe-in creating a **new** parameter defaults to `String` unless `--type` is given. On write the existing `keyId`/tier/description are preserved (fetched via `Describe`). Required IAM: `ssm:GetParameter`, `ssm:GetParameterHistory`, `ssm:PutParameter` (+ KMS for SecureString).
