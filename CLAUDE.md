...
**File edit** (`runFileEdit`): Download -> binary check -> temp file -> editor -> diff -> confirm -> upload. An `apierror.IsObjectNotFound` download means the key is a create target, so the flow hands off to `createObject` — `runNewFile` minus the up-front `HeadObject`, which the 404 already answered. `IsObjectNotFound` is narrower than `KindNotFound` on purpose: the latter also covers `NoSuchBucket`/`ContainerNotFound`, and a mistyped bucket must fail immediately rather than after the user has typed out a file. It is an allow-list (`NoSuchKey`, `BlobNotFound`), so an unrecognized not-found aborts too; the GCS sentinels are mapped onto those codes in `inspect`. `--meta` on a missing object still errors: there is nothing to attach metadata to.
...
**SSM Parameter Store** (`cmd/ladle/ssm.go`, `runSSM`): early-branch dispatcher for `ssm://`, mirroring the S3 flows (edit / pipe out / pipe in / list / versions / meta) against `internal/ssm.Client`. Differences from S3: no bucket (the URI's `Key` is the full parameter name, always leading-slash normalized). Interactive directory URIs (and namespace prefixes) open the **same TUI browser** as S3 via `ssmStorageAdapter` (`cmd/ladle/ssm_browser.go`), a thin `storage.Client` shim over `ssm.Client` — the browser works in S3-style keys (no leading slash), the adapter prepends `/` for SSM calls, and `browser.SetBucketListEnabled(false)` makes an empty bucket list the root instead of buckets. SecureString values are masked in the browser's version preview unless `--reveal`. When stdout is redirected, a directory/namespace prints a sorted listing (one canonical `ssm://` URI per line) and `--versions` prints tab-separated history. `internal/ssm.Client.Describe` reads metadata via `GetParameterHistory` (ARN-scopable IAM + strongly consistent), NOT `DescribeParameters` (account-wide + eventually consistent). Because SSM has no metadata-only write API, `--meta` re-`Put`s the current value alongside the new attributes; `Put` only sends `KeyId` for SecureString. **SecureString safety (`--reveal`):** value-exposing operations (edit, pipe-out, update diff) refuse for SecureString unless `--reveal` is passed; `--yes` writes skip the diff and don't need `--reveal`, but the no-op check still runs for non-secure values. Editor-based writes (`runSSMEdit`, `runSSMNewFile`) run the value through `trimEditorNewline` before the diff, stripping the trailing `\n`/`\r\n` an editor appends on save (SSM stores values verbatim, so a stray newline corrupts secrets); pipe-in keeps stdin's exact bytes. Interactive edit of a name that is neither a parameter nor a namespace falls through to `createSSMParam` (the `notFound` flag carried down from the namespace probe in `runSSM`; `createSSMParam` is `runSSMNewFile` minus the redundant `ensureParamAbsent`, which the probe already answered), so a direct URI creates just like the browser's `n` key. Pipe-in creating a **new** parameter defaults to `String` unless `--type` is given; `runSSMNewFile` instead resolves the type *after* the editor closes — browser choice, else `--type`, else `promptParamType` asks (`--yes` takes the `String` default rather than prompting). That prompt and the create confirmation share one `bufio.Scanner` over stdin (`confirmScan`), because a second scanner would lose whatever the first buffered past its line. `promptParamType` re-asks on a bad answer instead of returning — by then the caller holds a value the user typed — and every failure after the editor closes (EOF at the prompt, the pre-write re-check, the `Put`) leaves the temp file in place with a `Recovery:` line via a deferred check on the named `err` result, the way an editor failure does. On write the existing `keyId`/tier/description are preserved (fetched via `Describe`), except that `--description` overrides it: unlike `--type` (create-only, with a `Note:` when ignored), a description is mutable after creation, so `applyDescription` is called at every value-write site (`runSSMEdit`, `runSSMPipeIn`, `runSSMNewFile`, `runSSMRestoreVersion`) but never on the `--meta` flows, where the YAML is the sole authority (a `--description` there prints a `Note:` and is dropped). Interactive creation asks for the description right after the type, via `promptDescription` on the same shared scanner; `--yes` or a `--description` value skips the question. Because a description change is a change in its own right, the no-op guards in `runSSMEdit`/`runSSMPipeIn` compare it alongside the value diff, and `printDescriptionChange` shows it before the confirmation (the diff covers the value alone). `browserFlags` strips `--description` before the browser's callbacks see it: the flag names one parameter on the command line, while the browser acts on whatever the cursor is on. Prompting is deliberately limited to type and description — tier/keyId/dataType go through `--meta` or flags, so the create flow does not grow into a wizard. Required IAM: `ssm:GetParameter`, `ssm:GetParametersByPath`, `ssm:GetParameterHistory`, `ssm:PutParameter`, `ssm:DeleteParameter` (browser delete/move) (+ KMS for SecureString).
